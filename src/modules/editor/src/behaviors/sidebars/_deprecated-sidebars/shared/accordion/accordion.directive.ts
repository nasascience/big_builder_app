...
export class PebEditorAccordionDirective implements AfterViewInit, OnDestroy {
  ...
  @Input('pebEditorAccordionKey') containerName: any;
  ...
  toggleContainerDisplay() {
    if (this.container) {
      ...
      if (localStorage.getItem(`PEB-SIDEBAR-OPENED-${this.containerName}`) === 'true') {
  ...
  handleClick() {
    ...
    localStorage.setItem(`PEB-SIDEBAR-OPENED-${this.containerName}`, JSON.stringify(this.showContainer));
    this.toggleContainerDisplay();
  }
